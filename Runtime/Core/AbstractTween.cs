...
namespace EasyToolkit.Tweening
{
    ...
    public abstract class AbstractTween
    {
        ...
        internal TweenState CurrentState => _state.CurrentStateKey;
        ...
        protected virtual void OnStateChanged(TweenState previousState, TweenState newState)
        {
            switch (newState)
            {
        ...
        internal void Update()
        {
            ...
            Assert.IsTrue(stateKey != TweenState.Idle);

            if (stateKey == TweenState.Completed)
            {
                return;
            }

            if (stateKey != TweenState.Paused)
            {
            ...
            if (stateKey == TweenState.DelayAfterPlay || stateKey == TweenState.Playing)
            {
                if (_pause)
                {
                ...
                if (stateKey == TweenState.DelayAfterPlay)
                {
                    if (_playElapsedTime > Delay)
                    {
                        _state.ChangeState(TweenState.Playing);
